...
void OnlineIntersectDensePruned::Intersect(std::shared_ptr<DenseFsaVec> &b_fsas,
  ...
  */
  ...
  K2_EVAL(
      c_, num_seqs_, lambda_final_t, (int32_t i)->void {
  ...
  { /*  This block populates `final_arcs_shape`.  It is the shape of a ragged
    ...
    K2_EVAL(
        c_, num_fsas, lambda_set_num_extra_states, (int32_t i)->void {
          // int32_t b_chunk_size =
          // b_fsas_row_splits1[i + 1] - b_fsas_row_splits1[i];
          // int32_t b_final_t = final_t_data[i];
          // int32_t final_t = b_final_t + b_chunk_size - 1;
          // final_t_data[i] = final_t;
          // final_t = b_chunk_size;
          int32_t final_t = final_t_data[i];
          K2_LOG(INFO) << "final t : " << final_t;
