...
Ragged<int32_t> GetBatches(FsaVec &fsas, bool transpose) {
 ...
 // i, the batch_starts for that FSA begin at element fsa.RowSplits(1)[i] of
 ...
 // TODO(Dan): after debugging this version, change it to 0 and debug the more
 // complex version.
#if 1
 ...
 Eval(c, num_fsas, lambda_set_batch_info_simple);
  ...
  auto lambda_set_batch_info = [=] __host__ __device__ (int32_t fsa_idx,
                                                       int32_t j) {
    if (j % jobs_multiple != 0) return;  // a trick to avoid too much random
    ...
    // task_idx, task_idx + jobs_per_fsa, task_index + 2 * job_per_fsa and so on,
    // for the FSA numbered `fsa_idx`.
    // Comparing this code to `lambda_set_batch_info_simple`, this task is
    // responsible for the assignment to batch_starts_data for all i such
    // that i % jobs_per_fsas == task_idx, together with the
    // assignment to num_batchess_per_fsa_data if
    ...
    while (1) {
      batch_starts_data[begin_state_idx01 + i] = cur_state_idx01;
      int32_t next_state_idx01 = dest_states_powers_acc(log_power, cur_state_idx01);
      if (next_state_idx01 >= end_state_idx01) {
        // if exactly one step would also be enough to take us past the boundary...
  ...
  auto lambda_set_ans_row_ids2 = [=] __host__ __device__ (int32_t idx01) -> void {
    int32_t idx0 = ans_row_ids1_data[idx01],  // Fsa index
        ...
        idx1 = idx01 - idx0x,
        fsas_idx0x = fsas_row_splits1_data[idx0],  // 1st state-idx (idx01) in
                                                   // fsas_, for this FSA
