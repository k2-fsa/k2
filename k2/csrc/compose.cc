 ...
 public:
  ...
  void Intersect() {
    /*
      ...
      neural-net output.
    */
    int32_t T = b_fsas_.MaxSize(1), num_fsas = b_fsas_.Dim0();
  ...
  */
  Array1<float> GetPruningCutoffs(const Ragged3<float> &arc_end_scores) {
    ...
    auto lambda_set_beam_and_cutoffs =
        __host__ __device__[=](int32_t i)->float {
  ...
  Ragged3<ArcInfo> GetUnprunedArcs(int32_t t, FrameInfo *cur_frame) {
    ...
    // 'cur_frame->states')
    __host__ __device__ auto num_arcs_lambda =
        [=](int32_t state_idx01) -> int32_t {
    ...
    const int32_t
        *ai_row_ids1 = ai_shape.RowIds(
            1),  // from state_idx01 (into `states` or `ai_shape`) -> fsa_idx0
        *ai_row_ids2 = ai_shape.RowIds(
            2),  // from arc_idx012 (into `ai_shape`) to state_idx01
            *ai_row_splits2 =
                ai_shape.RowSplits(2),  // from state_idx01 to arc_idx01x
                *a_fsas_row_splits2 =
                    a_fsas_.RowSplits(2);  // from state_idx01 (into
  ...
  FrameInfo *PropagateForward(int t, FrameInfo *cur_frame) {
    ...
    Ragged<float> ai_loglikes(arc_info.shape,
                              Array1<float>(
                                  c_, arc_info.values.Dim(),
                                  __host__ __device__[=](int32_t i)->float {
                                    return ai_data[i].end_loglike;
                                  }));
    ...
    int32_t state_map_stride =
        (a_fsas_.Dim0() > 1 ? 0 : state_map_.valuestride0());
    auto lambda_set_state_map =
      ...
      float end_loglike = ai_data[i].end_loglike, cutoff = cutoffs_data[fsa_id];
    ...
    cur_frame->arcs =
        Ragged<Arc>(SubsampleRaggedShape(arc_info.shape, renumber_arcs);
    ...
    auto lambda_set_arcs_and_states =
      ...
      if (next_pruned_idx012 > this_pruned_idx012) {
        ...
        kept_states_data[state_idx01].a_fsas_state_idx01 =
            info.u.dest_a_fsas_state_idx01 info.u.dest_info_state_idx01 =
                state_idx01;
