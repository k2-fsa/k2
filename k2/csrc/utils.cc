...
__device__ void FillValues(int32_t *data, int32_t num_values,
  ...
  FillValuesKernel<<<dim_block, dim_grid>>>(data, num_values, value);
}
...
static int32 RoundUpToNearestPowerOfTwo(int32 n) {
  KALDI_ASSERT(n > 0);
  n--;
  n |= n >> 1;
  n |= n >> 2;
    ...
    if (1) {
      ...
      K2_CUDA_SAFE_CALL(
          RowSplitsToRowIdsKernel<<<dim_block, dim_grid, 0, c->GetCudaStream()>>>(
              num_rows, threads_per_row, row_splits, num_elems_row_ids));
    } else {
...
__global__ void RowIdsToRowSplitsKernel(
  ...
  K2_ASSERT(num_threads / threads_per_elem >= num_elems);
  ...
  if (d == kCpu) {
    ...
    if (no_empty_rows) {
      auto lambda_simple = [=] __host__ __device__ (int32_t i) {
        ...
        K2_ASSERT(this_row <= prev_row + 1); // no_empty_rows was asserted by
                                             // the user
