...
__global__ void histogram_pre_scan_compaction(key_t *input, uint32_t *bin,
  ...
  if (blockIdx.x ==
      (gridDim.x -
       1))  // last block, potentially may try to read invalid inputs
  {
...
#pragma unroll
    for (int kk = 0; kk < DEPTH; kk++) {
      ...
      // computing histogram
      uint32_t rx_buffer =
          __ballot_sync(0xFFFFFFFF, bucket_identifier(myInput[kk]));
      uint32_t myHisto =
          0xFFFFFFFF & (((laneId)&0x01) ? rx_buffer : (0xFFFFFFFF ^ rx_buffer));
      binCounter[kk] = __popc(myHisto & mask);

      // === storing the results into the shared memory
      if (laneId < 2) {
        // new hierarchy: Bucket -> warp -> roll
        scratchPad[laneId * NUM_W * DEPTH + warpId * DEPTH + kk] =
            binCounter[kk];
      }
    }
    __syncthreads();
...
#pragma unroll
    while (tid < (2 * NUM_W * DEPTH)) {
      ...
      uint32_t finalIndex =
          blockIdx.x * DEPTH * NUM_W + whatWarp * DEPTH + whatRoll;
      bin[whatBin * NUM_W * DEPTH * gridDim.x + finalIndex] = scratchPad[tid];
      // updating tid
      tid += blockDim.x;
    }
  } else  // all other blocks, that we are sure they are certainly processing
          // valid inputs:
  {
    // === computing the histogram:
    key_t myInput[DEPTH];
  ...
  while (tid < 2 * NUM_W * DEPTH) {
    ...
    warp_offsets_smem[threadIdx.x] =
        warpOffsets[(whatBin * NUM_W * gridDim.x * DEPTH) +
                    (blockIdx.x * NUM_W * DEPTH) + (whatWarp * DEPTH) +
                    whatRoll];
    tid += blockDim.x;
  }
...
#pragma unroll
    for (int kk = 0; kk < DEPTH; kk++) {
      ...
      uint32_t myMask =
          0xFFFFFFFF & ((myBucket) ? rx_buffer : (0xFFFFFFFF ^ rx_buffer));
      uint32_t myHisto =
          0xFFFFFFFF & (((laneId)&0x01) ? rx_buffer : (0xFFFFFFFF ^ rx_buffer));
      ...
      myNewIndex[kk] = (valid_input)
                           ? myNewIndex[kk]
                           : 32;  // if 32 it means that input was not valid
    }
...
#pragma unroll
    for (int kk = 0; kk < DEPTH; kk++) {
      keys_ms_smem[(warpId << 5) * DEPTH + (kk << 5) +
                   ((myNewIndex[kk] < 32) ? myNewIndex[kk] : laneId)] =
          myInput[kk];
    }
    __syncthreads();
...
#pragma unroll
    for (int kk = 0; kk < DEPTH; kk++) {
      ...
      uint32_t finalIndex =
          (valid_input)
              ? warp_offsets_smem[NUM_W * DEPTH * bucket_identifier(myNewKey) +
                                  warpId * DEPTH + kk] +
                    laneId
              : 0;
      finalIndex -= __shfl_sync(0xffffffff, scan_histo[kk],
                                bucket_identifier(myNewKey), 32);

      if (valid_input) key_output[finalIndex] = myNewKey;
    }
  } else {
// === Histogram and local index computation
#pragma unroll
...
#pragma unroll
    for (int kk = 0; kk < DEPTH; kk++) {
      ...
      uint32_t finalIndex =
          warp_offsets_smem[NUM_W * DEPTH * myNewBucket + warpId * DEPTH + kk] +
          laneId;
...
#pragma unroll
      for (int i = 0; i < LOG_B; i++) {
        ...
        myHisto = myHisto & (((laneId >> i) & 0x01) ? rx_buffer
                                                    : (0xFFFFFFFF ^ rx_buffer));
        bit >>= 1;
      }
      binCounter[kk] = __popc(myHisto & mask);
      // === storing the results into the shared memory
...
#pragma unroll
    for (int kk = 0; kk < DEPTH; kk++) {
      ...
      uint32_t myNewBucket = bucket_identifier(myNewKey);
      uint32_t finalIndex =
          (valid_input) ? warp_offsets_smem[NUM_W * DEPTH * myNewBucket +
                                            warpId * DEPTH + kk] +
                              laneId
                        : 0;
      finalIndex -= __shfl_sync(0xffffffff, scan_histo[kk], myNewBucket, 32);
...
__global__ void multisplit_WMS_pairs_postscan(
  ...
  uint32_t *warp_offsets_smem = scratchPad;
  key_type *keys_ms_smem = &warp_offsets_smem[NUM_B * NUM_W * DEPTH];
  value_type *values_ms_smem =
      &warp_offsets_smem[32 * NUM_W * DEPTH + NUM_B * NUM_W * DEPTH];
...
#pragma unroll
    for (int kk = 0; kk < DEPTH; kk++) {
      uint32_t global_index = blockIdx.x * blockDim.x * DEPTH +
                              ((kk * NUM_W) << 5) + (warpId << 5) + laneId;
      uint32_t myBucket = 0;
      bool valid_input = false;
...
#pragma unroll
      ...
      if (laneId < NUM_B) {
        ...
        scratchPad[kk * NUM_W * NUM_B + warpId * NUM_B + laneId] =
            binCounter[kk];
      }
    }
    __syncthreads();
...
#pragma unroll
      for (int kk = 0; kk < DEPTH; kk++) {
        if ((warpId & ((1 << i) - 1)) == 0) {
          if (laneId < NUM_B) {
            scratchPad[laneId + warpId * NUM_B + kk * NUM_W * NUM_B] +=
                scratchPad[laneId + (warpId + (1 << (i - 1))) * NUM_B +
                           kk * NUM_W * NUM_B];
          }
        }
      }
    ...
    // === storing the results back to the global memory with different
    // hierarchy
    uint32_t tid = threadIdx.x;
#pragma unroll
    while (tid < DEPTH * NUM_B) {
      ...
      bin[bucket_src * gridDim.x * DEPTH + blockIdx.x * DEPTH + roll_src] =
          scratchPad[bucket_src + roll_src * NUM_W * NUM_B];
      tid += blockDim.x;
    }
    ...
    if (NUM_W >= DEPTH) {
      if ((laneId < NUM_B) && (warpId < DEPTH))
        bin[laneId * gridDim.x * DEPTH + blockIdx.x * DEPTH + warpId] =
            scratchPad[laneId + warpId * NUM_B * NUM_W];
    }
  }
}
//==================================
...
__global__ void multisplit_BMS_postscan(key_type *key_input,
                                        ...
                                        uint32_t numElements,
                                        bucket_t bucket_identifier) {
  __shared__ uint32_t scratchPad[2 * NUM_B * DEPTH + 32 * NUM_W * DEPTH +
                                 NUM_B * NUM_W * DEPTH];
...
#pragma unroll
  while (tid < NUM_B * DEPTH) {
    uint32_t bucket_src = tid % NUM_B;
    uint32_t roll_src = tid / NUM_B;
    block_offsets_smem[bucket_src + roll_src * NUM_B] =
...
#pragma unroll
      ...
      if (laneId < NUM_B)
        scan_histo_smem[laneId + warpId * NUM_B + kk * NUM_B * NUM_W] =
            binCounter[kk];
      __syncthreads();
      ...
      for (int i = 1; i < (1 << LOG_W); i <<= 1) {
        if (laneId < NUM_B)
          scan_temp[kk] +=
              ((warpId >= i) ? scan_histo_smem[kk * NUM_B * NUM_W +
                                               (warpId - i) * NUM_B + laneId]
                             : 0);
        __syncthreads();
        if (laneId < NUM_B)
          scan_histo_smem[kk * NUM_B * NUM_W + warpId * NUM_B + laneId] =
              scan_temp[kk];
        __syncthreads();
      }
      // Computing block-level indices:
      scan_temp[kk] -= binCounter[kk];  // exclusive scan
      uint32_t myLocalBlockIndex =
          __shfl_sync(0xffffffff, scan_temp[kk], myBucket, 32);
      myLocalBlockIndex +=
          __popc(myLocalIndex & (0xFFFFFFFF >> (31 - laneId))) - 1;
...
#pragma unroll
      ...
      uint32_t myNewBlockIndex =
          warp_offsets_smem[myBucket + kk * NUM_B] + myLocalBlockIndex;
      // block-level reordering in shared memory
      keys_ms_smem[((myLocalBlockIndex < blockDim.x) ? myNewBlockIndex
                                                     : threadIdx.x) +
                   kk * blockDim.x] = myInput[kk];
    }
    __syncthreads();
#pragma unroll
    for (int kk = 0; kk < DEPTH; kk++) {
      ...
      key_type myNewKey = (valid_input)
                              ? keys_ms_smem[threadIdx.x + kk * blockDim.x]
                              : 0xFFFFFFFF;
      uint32_t myNewBucket = bucket_identifier(myNewKey);
...
#pragma unroll
    for (int kk = 0; kk < DEPTH; kk++) {
      ...
      uint32_t myNewBucket = bucket_identifier(myNewKey);
      uint32_t finalIndex =
          block_offsets_smem[NUM_B * kk + myNewBucket] + threadIdx.x;
      finalIndex -= warp_offsets_smem[myNewBucket + kk * NUM_B];
      key_output[finalIndex] = myNewKey;
    }
  }
...
__global__ void multisplit_BMS_pairs_postscan(
    ...
    key_type *key_output, value_type *value_output, uint32_t numElements,
    bucket_t bucket_identifier) {
  __shared__ uint32_t scratchPad[2 * NUM_B * DEPTH + 64 * NUM_W * DEPTH +
                                 NUM_B * NUM_W * DEPTH];
  uint32_t *block_offsets_smem = scratchPad;
  uint32_t *warp_offsets_smem = &block_offsets_smem[NUM_B * DEPTH];
  uint32_t *scan_histo_smem = &warp_offsets_smem[NUM_B * DEPTH];
  key_type *keys_ms_smem = &scan_histo_smem[NUM_B * NUM_W * DEPTH];
  value_type *values_ms_smem =
      &scan_histo_smem[NUM_B * NUM_W * DEPTH + 32 * NUM_W * DEPTH];
...
#pragma unroll
      ...
      keys_ms_smem[((myLocalBlockIndex < blockDim.x) ? myNewBlockIndex
                                                     : threadIdx.x) +
                   kk * blockDim.x] = myInput[kk];
      values_ms_smem[((myLocalBlockIndex < blockDim.x) ? myNewBlockIndex
                                                       : threadIdx.x) +
                     kk * blockDim.x] = myValue[kk];
    }
    __syncthreads();
#pragma unroll
    for (int kk = 0; kk < DEPTH; kk++) {
      ...
      key_type myNewKey = (valid_input)
                              ...
                              : 0xFFFFFFFF;
      value_type myNewValue =
          (valid_input) ? values_ms_smem[threadIdx.x + kk * blockDim.x]
                        : 0xFFFFFFFF;
      uint32_t myNewBucket = bucket_identifier(myNewKey);
      uint32_t finalIndex = 0;
      if (valid_input) {
