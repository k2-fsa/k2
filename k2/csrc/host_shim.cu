...
void FsaVecCreator::Init(const std::vector<k2host::Array2Size<int32_t>> &sizes) {
...
void FsaVecCreator::FinalizeRowSplits2() {
  ...
  for (int32_t i = 0; i < num_fsas; i++) {
    ...
    K2_CHECK(row_splits2_data[begin_state] == 0 ||
             num_states == 0);
...
FsaVec FsaVecCreator::GetFsaVec() {
  FinalizeRowSplits2();
  return Ragged<Arc>(RaggedShape3(&row_splits1_, nullptr, -1,
                                  &row_splits2_, nullptr, -1),
                     arcs_);
}
