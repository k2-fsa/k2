...
class TopSorter {
 public:
  ...
  /*
    Create the IterInfo object for the 1st iteration of the algorithm (not
    including its `dest_states` member).
   */
  std::unique_ptr<IterInfo> GetFirstIterInfo() {
    ...
    auto lambda_set_num_initial_states = [=] __host__ __device__(int32_t i) {
    ...
    ans->states = Ragged<int32_t>(
        RaggedShape2(&ans_states_row_splits1, &ans_states_row_ids1, num_elems),
        ans_states_values);
    return ans;
  }

  /*
    Assuming the `states` and `offsets` fields of cur_iter have already been set
    up, set up `cur_iter.dest_states` and returns a ragged array with 2 axes
    consisting of any new state-ids that were not previously encountered, that
    need to have arcs leaving them processed.  (These will become the `states`
    element of the next IterInfo, containing idx01's into fsas_).
  ...
  std::unique_ptr<IterInfo> ComputeDestStates(IterInfo &cur) {
    ...
    cur.dest_states = Ragged<int32_t>(
        ComposeRaggedShapes(
            cur.states.shape,
            RaggedShape2(&dest_state_row_splits2, nullptr, tot_arcs)),
    ...
    const int32_t *dest_states_row_ids2_data =
                      cur.dest_states.shape.RowIds(2).Data(),
                  *dest_states_row_splits2_data =
                      cur.dest_states.shape.RowSplits(2).Data(),
                  *dest_states_row_ids1_data =
                      cur.dest_states.shape.RowIds(1).Data(),
    ...
    int32_t *dest_states_data = cur.dest_states.values.Data(),
            *state_map_data = state_map_.Data(), *is_new_data = is_new.Data();
    ...
    auto lambda_set_dest_states1 = [=] __host__ __device__(int32_t arc_idx012) {
      ...
      int32_t fsas_state_idx01 = states_data[state_idx01],
              ...
              fsas_dest_state_idx01 =
                  fsas_arcs_data[fsas_arc_idx012].dest_state,
    ...
    auto lambda_set_new_states_row_splits1 =
        [=] __host__ __device__(int32_t fsa_idx) -> void {
      int32_t state_idx0x = states_row_splits1_data[fsa_idx],
              arc_idx0xx = dest_state_row_splits2_data[state_idx0x],
              // `is_new_data` currently contains the exclusive-sum of the
              // `is_new` info.
    ...
    auto lambda_set_new_states_etc =
        [=] __host__ __device__(int32_t arc_idx012) -> void {
      int32_t dest = dest_states_data[arc_idx012];
      if (dest < -1) {
        // The purpose of the next two lines is to work out fsa_idx0 (the FSA
        // index) so we can set the row_ids of `ans`
        int32_t state_idx01 = dest_states_row_ids2_data[arc_idx012],
                fsa_idx0 = dest_states_row_ids1_data[state_idx01];

        // note: this_arc_idx012 may or may not be equal to arc_idx012,
        // depending whether this arc's thread 'won the race' in a previous
        // lambda.
        ...
        int32_t new_state_idx0x = new_states_row_splits1_data[fsa_idx0],
                ...
                new_state_idx1_global =
                    state_offsets_data[fsa_idx0] + new_state_idx1;
    ...
    auto lambda_increase_state_offsets =
  ...
  */
  std::unique_ptr<IterInfo> GetLastIterInfo() {
    ...
    auto lambda_set_num_final_states =
        [=] __host__ __device__(int32_t fsa_idx0) {
          int32_t num_final_states =
              (fsas_row_splits1[fsa_idx0 + 1] > fsas_row_splits1[fsa_idx0]);
    ...
    ans->states = Ragged<int32_t>(
        RaggedShape2(&states_row_splits1, nullptr, tot_final_states),
    ...
    auto lambda_set_ans_states =
        [=] __host__ __device__(int32_t ans_states_idx01) {
          ...
          // Note: this would actually only ever be accessed in checking code,
          // as this state has no arcs leaving it.
          int32_t fsas_state_idx01 = fsas_row_splits1[fsa_idx0 + 1] - 1;
          ans_states_data[ans_states_idx01] = fsas_state_idx01;
        };

    // These final states all have zero arcs leaving them so ans->dest_states
    // has no elements.
    Array1<int32_t> temp_row_splits(c_, tot_final_states + 1, 0),
        temp_row_ids(c_, 0);
    RaggedShape dest_states_shape_part =
        RaggedShape2(&temp_row_splits, &temp_row_ids, 0);
    ans->dest_states = Ragged<int32_t>(
        ComposeRaggedShapes(ans->states.shape, dest_states_shape_part),
        Array1<int32_t>(c_, 0));
    return ans;
  }
  ...
  FsaVec FormatOutput(Array1<int32_t> *arc_map) {
    int32_t num_fsas = fsas_.Dim0(), num_iters = iters_.size();
    ...
    std::vector<const int32_t *> dest_states_ptrs_vec(num_iters),
        ...
        dest_states_row_splits2_ptrs_vec(num_iters), states_ptrs_vec(num_iters);
    ...
    for (size_t i = 0; i < num_iters; i++) {
      ...
      dest_states_row_splits2_ptrs_vec[i] =
          iters_[i]->dest_states.RowSplits(2).Data();
    ...
    const int32_t **dest_states_ptrs_data = dest_states_ptrs.Data(),
                  **states_row_splits1_ptrs_data =
                      states_row_splits1_ptrs.Data(),
                  **dest_states_row_splits2_ptrs_data =
                      dest_states_row_splits2_ptrs.Data(),
    ...
    auto lambda_set_arcs = [=] __host__ __device__(int32_t arc_idx0123) {
      ...
      // more axis than `ans` (i.e. the `iter` axis).  so
      // 0123==(fsa,iter,state,arc), where the state and arc indexes are
      // respectively idx1's into IterInfo::{state,dest_state} and idx2's into
      // IterInfo::dest_state.
      int32_t
          state_idx012 = stacked_row_ids3_data[arc_idx0123],
          ...
          state_idx2 = state_idx012 -
                       fsaiter_idx01x,  // state index given (iter and FSA)
          arc_idx23 =
              arc_idx0123 - fsaiter_idx01xx,  // arc index given (iter and FSA)
          arc_idx3 = arc_idx0123 - state_idx012x;

      // `states_fsa_idx0x` is the idx0x into the `states` and `dest_states`
      // tensors for the iteration corresponding to `iter_idx`, for this
      // fsa_idx0.
      int32_t
          states_fsa_idx0x = states_row_splits1_ptrs_data[iter_idx1][fsa_idx0],
      ...
      int32_t fsas_state_idx01 =
                  states_ptrs_data[iter_idx1][states_fsa_idx0x + state_idx2],
              fsas_state_idx01x = fsas_row_splits2_data[fsas_state_idx01],
              fsas_arc_idx012 = fsas_state_idx01x + arc_idx3;

      // below, the idx0xx + idx23 is not an error though it may look like
      // one... the arc_idx23 into `stacked_shape` corresponds to arc_idx12 into
      // IterInfo::dest_states.
      // here `dest_state_idx1` is the state_idx1 into the output FsaVec.
      int32_t dest_state_idx1 =
          dest_states_ptrs_data[iter_idx1][dest_states_fsa_idx0xx + arc_idx23];

      Arc arc = fsas_arcs_data[fsas_arc_idx012];
      arc.src_state =
          state_idx12;  // state_idx12 into `stacked` == state_idx1 into `ans`
