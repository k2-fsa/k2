...
class TopSorter {
 public:
   ...
   */
  std::unique_ptr<IterInfo> ComputeDestStates(IterInfo &cur_iter) {
    ...
    int32_t num_arcs = dest_state_row_splits2[num_states];

    cur_iter.dest_states = Ragged<int32_t>(
        ComposeRaggedShapes(cur_iter.states.shape,
                            Ragged2(dest_state_row_splits2, nullptr, num_arcs)),
    ...
    const int32_t *dest_states_row_ids2_data = cur_iter.dest_states.shape.RowIds(2).Data(),
        *dest_states_row_splits2_data = cur_iter.dest_states.shape.RowSplits(2).Data(),
        *dest_states_row_ids1_data = cur_iter.dest_stats.shape.RowIds(1).Data(),
        *fsas_row_splits2_data = fsas_.shape.RowSplits(2);
    ...
    auto lambda_set_dest_states1 = [=] __host__ __device__ (int32_t i) {
       int32_t arc_idx012 = i;  // index into cur_iter.dest_states
       ...
       int32_t fsa_state_idx01 = states_data[state_idx01],
           fsa_arc_idx01x = fsas_row_splits2_data[fsa_state_idx01],
           fsa_arc_idx012 = fsa_arc_idx01x + arc_idx2,
           fsa_dest_state_idx01 = arcs_data[fsa_arc_idx012].dest_state,
           symbol = arcs_data[fsa_arc_idx012].symbol;
       if (symbol == -1) {
    ...
    Eval(c_, num_arcs, lambda_set_is_new);
    ...
    int32_t num_new_states = is_new[num_arcs];
    ...
    std::unique_ptr<IterInfo> ans = std::make_unique<IterInfo>();
    ans->offsets = Clone(state_offsets_);

    ans->states = RaggedFromTotSizes<int32_t>(c_, {num_fsas, num_new_states});

    int32_t *new_states_row_splits1_data = ans->states.RowSplits(1).Data();
    const int32_t *states_row_splits1_data = cur_iter.states..RowSplits(1).Data(),
    ...
    auto lambda_set_new_states_etc = [=] __host__ __device__ (int32_t arc_idx012) -> void {
       ...
       if (dest < -1) {
         ...
         // `next_state_idx01` is an index into `ans` which will become
         // the next IterInfo::states.
         int32_t next_state_idx01 = is_new_data[this_arc_idx012];

         new_states_row_ids1_data[next_state_idx01] = fsa_idx0;
         int32_t fsa_next_state_idx01 = arcs[arc_idx012].dest_state;
         new_states_values_data[next_state_idx01] = fsa_next_state_idx01;
         int32_t new_states_idx0x = new_states_row_splits1_data[fsa_idx0],
             new_states_idx1 = new_states_idx01 - new_states_idx0x,
             new_states_idx1_global = state_offsets_data[fsa_idx0] +
                   new_states_idx1;
         // `global` means: not just on this iter, the idx1 within the returned
         // FSA.
         dest_states_data[arc_idx012] = new_states_idx1_global;
         state_map_data[fsa_new_states_idx01] = new_states_idx1_global;
       }
    };
    Eval(c_, num_arcs, lambda_set_new_states_etc);
    ...
    auto lambda_increase_state_offsets = [=] __host__ __device__ (int32_t fsa_idx0) -> void {
      int32_t num_new_states = ans_row_splits1_data[fsa_idx0+1] -
          ans_row_splits1_data[fsa_idx0];
  ...
  */
  std::unique_ptr<IterInfo> GetLastIterInfo() {
    ...
    Array1<int32> &states_row_splits1 = num_final_states;
    int32_t tot_final_states = states_row_splits1[num_fsas];

    ans->states = Ragged<int32_t>(Ragged2(&states_row_splits1,
                                          nullptr, tot_final_states),
                                  Array1<int32_t>(c_, tot_final_states));
    const int32_t *fsas_row_splits1 = fsas_.RowSplits(1),
    ...
    auto lambda_set_ans_states = [=] __host__ __device__ (int32_t ans_states_idx01) {
       int32_t fsa_idx0 = ans_row_ids1_data[ans_states_idx01];
       // last state of input FSA for this state-index.  Note: this would
       // actually only ever be accessed in checking code, as this state has no
       // arcs leaving it.
    ...
    // elements.
    Ragged dest_states_part = Ragged2(Array1<int32_t>(c_, tot_final_states + 1, 0),
                                      Array1<int32_t>(c_, 0));
    ans->dest_states = Ragged<int32_t>(ComposeRaggedShapes(ans->states,
                                                           dest_states_part),
                                       Array1<int32_t>(c_, 0));
    return ans;
  }
  ...
  FsaVec FormatOutput(Array1<int32_t> *arc_map) {
    ...
    std::vector<RaggedShape*> dest_states_shapes(num_iters);
    ...
    for (size_t i = 0; i < num_iters; i++) {
      dest_states_shapes[i] = &(iters_[i]->dest_states);
    ...
    Array1<const int32_t*> dest_states_ptrs(dest_states_ptrs_vec),
        states_row_splits1_ptrs(states_row_splits1_ptrs_vec),
        dest_states_row_splits2_ptrs(dest_states_row_splits2_ptrs_vec),
        states_ptrs(states_ptrs_vec);
    const int32_t **dest_states_ptrs_data = dest_states_ptrs.Data(),
      **states_row_splits1_ptrs_data = states_row_splits1_ptrs.Data(),
      **dest_tates_row_splits1_ptrs_data = dest_states_row_splits2_ptrs.Data(),
    ...
    auto lambda_set_arcs = [=] __host__ __device__ (int32_t arc_idx0123) {
      ...
      // more axis than `ans` (i.e. the `iter` axis).
      int32_t state_idx012 = stacked_row_ids3_data[arc_idx0123],
          ...
          fsaiter_idx0xx = ans_row_splits1_data[fsa_idx0], // ==stacked_row_splits2[fsa_idx0xx].
          state_idx12 = state_idx012 - state_id0xx,  // == state_idx1 into `ans`.
          ...
          arc_idx3 = arc_idx012 - state_idx012x;

      // `states_fsa_idx0` is the idx0x into the `states` and `dest_states`
      ...
      int32_t states_fsa_idx0x = states_row_splits1_ptrs_data[iter_idx1][fsa_idx0],
          dest_states_fsa_idx0xx = dest_states_row_splits2_ptrs_data[states_fsa_idx0x];

      // OK, we want::
      //     - source-state and dest-state idx1's in ans
      //     - arc-index in fsas_
      ...
      if (arc.dest_state >= 0) {
        arc.dest_state = dest_state_idx1;  // i.e. idx1 into `ans`.
      } else {
      ...
      if (arc_map_data != nullptr) {
        arc_map_data[arc_idx0123] = fsa_arc_idx012;
      }
    };
    Eval(c_, num_arcs, lambda_set_arcs);
    return ans;
  }
...
void TopSort(FsaVec &src, FsaVec *dest, Array1<int32_t> *arc_map) {
  ...
  if (src.NumAxes() == 2) {
    ...
    FsaVec src_vec = CreateFsaVec(1, &src), dest_vec;
    // Recurse..
