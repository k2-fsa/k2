...
static BenchmarkStat BenchmarkGetTransposeReordering(int32_t dim,
  ...
  BenchmarkStat stat;
  stat.op_name = "GetTransposeReordering_" + std::to_string(num_states) + "_" +
                 std::to_string(num_arcs);
...
static void RegisterBenchmarkGetTransposeReordering(DeviceType device_type) {
  ...
  std::vector<int32_t> problems_sizes = {100, 500, 1000};
