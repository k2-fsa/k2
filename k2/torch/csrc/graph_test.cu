...
TEST(GraphTest, CtcTopo) {
  for (const ContextPtr &c : {GetCpuContext(), GetCudaContext()}) {
    ...
    FsaClass topo = CtcTopo(3, false, device);
    ...
    EXPECT_TRUE(
        torch::equal(topo.GetAttr("aux_labels").toTensor(), aux_label_ref));
...
TEST(GraphTest, CtcGraphs) {
  for (const ContextPtr &c : {GetCpuContext(), GetCudaContext()}) {
    auto device = GetDevice(c);
    RaggedAny symbols("[ [ 1 2 2 3 ] [ 1 2 3 ] ]", torch::kInt32, device);
    // test standard
    FsaClass graph = CtcGraphs(symbols, false);
    ...
    EXPECT_TRUE(
        torch::equal(graph.GetAttr("aux_labels").toTensor(), aux_labels_ref));
...
TEST(GraphTest, LinearFsaSingle) {
  for (const ContextPtr &c : {GetCpuContext(), GetCudaContext()}) {
    ...
    FsaClass fsa = LinearFsa(labels, device);
...
TEST(GraphTest, LinearFsaVec) {
  for (const ContextPtr &c : {GetCpuContext(), GetCudaContext()}) {
    ...
    RaggedAny labels_any("[[1 3 5] [2 6] [8 7 9]]", torch::kInt32, device);
...
TEST(GraphTest, LevenshteinGraphs) {
  for (const ContextPtr &c : {GetCpuContext(), GetCudaContext()}) {
    ...
    RaggedAny symbols("[[1 2 3] [] [4 5 6]]", torch::kInt32, device);
    FsaClass graph = LevenshteinGraphs(symbols, ins_del_score);
    ...
    EXPECT_TRUE(torch::equal(
        graph.Arcs().index(
            {"...", torch::indexing::Slice(torch::indexing::None, 3)}),
        expected_arcs));
    EXPECT_TRUE(torch::equal(graph.GetAttr("aux_labels").toTensor(),
                             expected_aux_labels));
    EXPECT_TRUE(torch::allclose(
        graph.GetAttr("__ins_del_score_offset_internal_attr_").toTensor(),
