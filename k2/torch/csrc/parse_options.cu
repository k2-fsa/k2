...
#ifdef _MSC_VER
#define K2_STRTOLL(cur_cstr, end_cstr) _strtoi64(cur_cstr, end_cstr, 10);
#else
#define K2_STRTOLL(cur_cstr, end_cstr) strtoll(cur_cstr, end_cstr, 10);
#endif
...
bool ConvertStringToInteger(const std::string &str, Int *out) {
  ...
  char *end = nullptr;
  errno = 0;
  int64_t i = K2_STRTOLL(this_str, &end);
  if (end != this_str) {
...
void ParseOptions::RegisterCommon(const std::string &name, T *ptr,
  ...
  K2_CHECK(ptr != nullptr);
  std::string idx = name;
  NormalizeArgName(&idx);
  if (doc_map_.find(idx) != doc_map_.end()) {
    K2_LOG(WARNING) << "Registering option twice, ignoring second time: "
                    << name;
  } else {
...
void ParseOptions::DisableOption(const std::string &name) {
  if (argv_ != nullptr) {
    K2_LOG(FATAL) << "DisableOption must not be called after calling Read().";
  }
  if (doc_map_.erase(name) == 0) {
    K2_LOG(FATAL) << "Option " << name
...
std::string ParseOptions::GetArg(int i) const {
  if (i < 1 || i > static_cast<int>(positional_args_.size())) {
    K2_LOG(FATAL) << "ParseOptions::GetArg, invalid index " << i;
  }
...
static bool MustBeQuoted(const std::string &str, ShellType st) {
  ...
  K2_CHECK_EQ(st, kBash) << "Invalid shell type.";
  ...
  if (*c == '\0') {
    ...
    K2_CHECK(!strchr(ok_chars[kBash], ' '));
  ...
  for (i = 1; i < argc; ++i) {
    if (std::strncmp(argv[i], "--", 2) == 0) {
      ...
      if (!SetOption(key, value, has_equal_sign)) {
        PrintUsage(true);
        K2_LOG(FATAL) << "Invalid option " << argv[i];
      }
    } else {
      break;
    }
...
void ParseOptions::PrintConfig(std::ostream &os) const {
  ...
  for (auto it = doc_map_.begin(); it != doc_map_.end(); ++it) {
    ...
    if (bool_map_.end() != bool_map_.find(key)) {
      ...
      K2_LOG(FATAL) << "PrintConfig: unrecognized option " << key
                    << "[code error]";
    }
    os << '\n';
  }
...
void ParseOptions::ReadConfigFile(const std::string &filename) {
  ...
  if (!is.good()) {
    K2_LOG(FATAL) << "Cannot open config file: " << filename;
  }
  ...
  while (std::getline(is, line)) {
    ...
    if (!SetOption(key, value, has_equal_sign)) {
      PrintUsage(true);
      K2_LOG(FATAL) << "Invalid option " << line << " in config file "
...
void ParseOptions::SplitLongArg(const std::string &in, std::string *key,
  ...
  K2_CHECK(in.substr(0, 2) == "--") << in;  // precondition.
  ...
  if (pos == std::string::npos) {  // we allow --option for bools
    ...
    *has_equal_sign = false;
    ...
    PrintUsage(true);
    K2_LOG(FATAL) << "Invalid option (no key): " << in;
...
bool ParseOptions::SetOption(const std::string &key, const std::string &value,
  ...
  if (bool_map_.end() != bool_map_.find(key)) {
    if (has_equal_sign && value == "") {
      K2_LOG(FATAL) << "Invalid option --" << key << "=";
    }
...
int32_t ParseOptions::ToInt(const std::string &str) const {
  ...
  if (!ConvertStringToInteger(str, &ret))
    K2_LOG(FATAL) << "Invalid integer option \"" << str << "\"";
  return ret;
}
...
float ParseOptions::ToFloat(const std::string &str) const {
  ...
  if (!ConvertStringToReal(str, &ret))
    K2_LOG(FATAL) << "Invalid floating-point option \"" << str << "\"";
  return ret;
}
