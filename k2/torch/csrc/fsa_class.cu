...
void FsaClass::CopyOtherAttrs(std::vector<FsaClass> &srcs) {
...
std::string FsaClass::ToString() /*const*/ {
  ...
  for (auto &p : tensor_attrs) {
    if (p.second.scalar_type() == torch::kInt) {
      extra_labels.emplace_back(FromTorch<int32_t>(p.second));
    }
  }
...
FsaClass FsaClass::AddEpsilonSelfLoops() /*const*/ {
  DeviceGuard guard(fsa.Context());
...
FsaClass FsaClass::ArcSort() /*const*/ {
  if (Properties() & kFsaPropertiesArcSorted) return *this;
  DeviceGuard guard(fsa.Context());
  Array1<int32_t> arc_map;
  Ragged<Arc> arcs;
...
FsaClass FsaClass::Connect() /*const*/ {
  if ((Properties() & kFsaPropertiesMaybeAccessible) &&
      (Properties() & kFsaPropertiesMaybeCoaccessible))
    return *this;
...
FsaClass FsaClass::TopSort() /*const*/ {
  if (Properties() & kFsaPropertiesTopSorted) return *this;
