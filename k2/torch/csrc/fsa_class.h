...
struct FsaClass {
  ...
  */
  ...
  FsaClass Connect() /*const*/;
  FsaClass TopSort() /*const*/;
  FsaClass AddEpsilonSelfLoops() /*const*/;
 ...
 private:
  ...
  void CopyTensorAttrs(std::vector<FsaClass> &srcs);

  // Propagate other attributes from src.
  void CopyOtherAttrs(FsaClass &src);
  void CopyOtherAttrs(std::vector<FsaClass> &srcs);
