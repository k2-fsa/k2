...
class TestRaggedOps(unittest.TestCase):
    ...
    def test_pad(self):
        ...
        for device in self.devices:
            for dtype in self.dtypes:
                ...
                value = value if dtype == torch.int32 else value * 1.0
                ans = src.pad('constant', value)
                expected = torch.tensor(
                    [[1, 2, value, value], [3, value, value, value],
                     [value, value, value, value], [4, 5, 6, value],
                     [7, 8, 9, 10]],
                    dtype=dtype,
                    device=device)
    ...
    def test_remove_values_leq(self):
        ...
        for device in self.devices:
            for dtype in self.dtypes:
                ...
                value = 0 if dtype == torch.int32 else 0.0
                ans = src.remove_values_leq(value)
                assert ans == k2.RaggedTensor(
                    '[ [ 1 2 ] [ 3 2 ] [ 8 6 ] [ ] ]',
                    device=device,
                    dtype=dtype)

                value = 1 if dtype == torch.int32 else 1.0
                ans = src.remove_values_leq(value)
                assert ans == k2.RaggedTensor('[ [ 2 ] [ 3 2 ] [ 8 6 ] [ ] ]',
                                              dtype=dtype,
                                              device=device)

                value = 6 if dtype == torch.int32 else 6.0
                ans = src.remove_values_leq(value)
                assert ans == k2.RaggedTensor('[ [ ] [ ] [ 8 ] [ ] ]',
                                              device=device,
                                              dtype=dtype)

                value = 8 if dtype == torch.int32 else 8.0
                ans = src.remove_values_leq(value)
                assert ans == k2.RaggedTensor('[ [ ] [ ] [ ] [ ] ]',
                                              dtype=dtype,
                                              device=device)
    ...
    def test_remove_values_eq(self):
        ...
        for device in self.devices:
            for dtype in self.dtypes:
                ...
                value = 0 if dtype == torch.int32 else 0.0
                ans = src.remove_values_eq(value)
                assert ans == k2.RaggedTensor(
                    '[ [ 1 2 ] [ 3 2 ] [ 8 6 ] [ ] ]',
                    device=device,
                    dtype=dtype)
    ...
    def test_normalize_scores_use_log_from_shape(self):
        ...
        '''
        for device in self.devices:
            fsa = k2.Fsa(s).to(device)
            scores = torch.arange(fsa.scores.numel(),
                                  dtype=torch.float32,
                                  device=device)
            scores.requires_grad_(True)

            ragged_scores = k2.RaggedTensor(fsa.raw_shape, scores)
            ...
            assert normalized_scores.values.requires_grad is True
    ...
    def test_sum_per_sublist(self):
        ...
        for device in self.devices:
            ...
            ragged = k2.RaggedTensor(fsa.raw_shape, fsa.scores.exp())
            normalized_scores = ragged.sum()
            assert normalized_scores.numel() == fsa.raw_shape.dim0
        ...
        for device in self.devices:
            for dtype in self.dtypes:
                src = k2.RaggedTensor(
                    [[1, 3, -1, -2], [1, 0, -1], [3, 2, 1], [], [1], [2, 3]],
                    dtype=dtype).to(device)
                initial_value = 0 if dtype == torch.int32 else 0.0
                indexes = src.max(initial_value=initial_value)
