...
#include "k2/python/csrc/mutual_information.h"
...
torch::Tensor mutual_information(torch::Tensor px,
                                     torch::Tensor py,
        ...
        for (int b = 0; b < B; b++) {
          ...
          for (int s = s_begin + 1; s <= s_end; ++s) {
            ...
            for (int t = t_begin + 1; t <= t_end; ++t) {
              ...
              p_a[b][s][t] = p_s_t1 = LogAdd<scalar_t>()(p_a[b][s - 1][t] + px_a[b][s - 1][t],
                                             p_s_t1 + py_a[b][s][t - 1]);
            }
          }
...
std::vector<torch::Tensor> mutual_information_backward(
    ...
    bool overwrite_ans_grad /* not used here */) {
  TORCH_CHECK(px.dim() == 3, "px must be 3-dimensional");
  TORCH_CHECK(py.dim() == 3, "py must be 3-dimensional.");
    TORCH_CHECK(p.dim() == 3, "p must be 3-dimensional.");
  TORCH_CHECK(boundary.dim() == 2, "boundary must be 2-dimensional.");
