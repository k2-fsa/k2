...
def get_rnnt_logprobs(lm: Tensor, am: Tensor, symbols: Tensor,
    ...
    """
    assert lm.ndim == 3 and am.ndim == 3 and lm.shape[0] == am.shape[
        0] and lm.shape[2] == am.shape[2]
    (B, T, C) = am.shape
    S = lm.shape[1] - 1
    ...
    px_am = torch.gather(am.unsqueeze(1).expand(B, S, T, C),
                         dim=3,
                         index=symbols.reshape(B, S, 1,
                                               1).expand(B, S, T, 1)).squeeze(
                                                   -1)  # [B][S][T]
    px_am = torch.cat(
        (px_am,
         torch.full(
             (B, S, 1), float('-inf'), device=px_am.device,
             dtype=px_am.dtype)),
...
def get_pruning_ranges(px_grad: torch.Tensor, py_grad: torch.Tensor,
                       s_range: int) -> torch.Tensor:
    """
    ...
    tot_grad = torch.cat((torch.zeros(
        (B, 1, T + 1), dtype=tot_grad.dtype,
        device=tot_grad.device), tot_grad),
                         dim=1)
...
def pruning(am: torch.Tensor, lm: torch.Tensor,
    ...
    # (B, T, S_range, C)
    lm_pruning = torch.gather(lm.unsqueeze(1).expand((B, T, S + 1, C)),
                              dim=2,
...
def pruning_rnnt_loss(joint: Tensor,
    ...
    Returns:
      ...
      of the batch (like log-probs of sequences).
    """
    px, py = get_rnnt_logprobs_pruning(joint, symbols, ranges,
                                       termination_symbol)
    return mutual_information_recursion(px, py, boundary)
...
def get_rnnt_logprobs_aux(lm: Tensor,
    ...
    unigram_lm = torch.mean(lm_probs / lmonly_normalizers,
                            dim=(0, 1),
                            keepdim=True) + 1.0e-20  # [1][1][C]
    amonly_normalizers = torch.mv(am_probs.reshape(
        -1, C), unigram_lm.reshape(C)).reshape(B, T,
                                               1).log() + am_max  # [B][T][1]
    amonly_normalizers = amonly_normalizers.transpose(1, 2)  # [B][1][T]
    unigram_lm = unigram_lm.log()
    lmonly_normalizers = lmonly_normalizers.log(
    ) + lm_max  # [B][S+1][1], log-normalizer, used for LM-only part of prob.
