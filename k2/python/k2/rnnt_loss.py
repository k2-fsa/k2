...
def adjust_pruning_lower_bound(s_begin: torch.Tensor,
    ...
    To make it monotonic increasing, we can `monotonic_lower_bound` function in
    k2, which guarantee `s_begin[i] <= s_begin[i + 1]`. The main idea is:
    ...
    have to make all the elements that less than 0 to 0 before transforming
    back the `s_begin`.
    """
    # s_begin (B, T)
    (B, T) = s_begin.shape
...
def get_rnnt_prune_ranges(px_grad: torch.Tensor, py_grad: torch.Tensor,
    ...
    Returns:
      A tensor contains the kept symbols id for each frame, with shape
      (B, T, s_range).
    """
...
def get_rnnt_logprobs_pruned(
    ...
    Args:
      joint:
        The pruned output of joint network, with shape (B, T, s_range, C)
      symbols:
        The symbol sequences, a LongTensor of shape [B][S], and elements in
        {0..C-1}.
      termination_symbol:
