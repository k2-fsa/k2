...
class Ragged(object):
    ...
    def __init__(
        self, ragged: Union[_k2.RaggedInt, _k2.RaggedFloat],
    ...
    def tot_sizes(self) -> List:
        """Return the elements numbers of all axises in a list, the length of
        the list equals to the `self.num_axes()`.
        """
    ...
    def index(self, indexes: Union[int, torch.Tensor, 'RaggedInt'],
            need_value_indexes: bool = False, axis: int = 0
        ) -> Union['Ragged', Tuple['Ragged', int],
                Tuple['Ragged', Optional[torch.Tensor]]]:
        """See :func:`k2.ragged.index."""
        if isinstance(indexes, torch.Tensor):
            ragged, value_indexes = self.ragged.index(
                axis, indexes, need_value_indexes
            )
            return Ragged(ragged), value_indexes
        if isinstance(indexes, int):
            ragged, offset = self.ragged.index(axis, indexes)
            return Ragged(ragged), offset

        assert isinstance(indexes.ragged, _k2.RaggedInt)
        return Ragged(self.ragged.index(indexes.ragged))

    def arange(self, axis: int, begin: int, end: int) -> "Ragged":
        """See :func:`k2.ragged.arange`."""
        return Ragged(self.ragged.arange(axis, begin, end))

    def max(self, initial_value: float = 0, axis: int = -1) -> torch.Tensor:
        """See :func:`k2.ragged.max`."""
        return _k2.max_per_sublist(self.ragged, initial_value, axis)

    def min(self, initial_value: float = 0, axis: int = -1) -> torch.Tensor:
        """See :func:`k2.ragged.min`."""
        return _k2.min_per_sublist(self.ragged, initial_value, axis)

    def logsum(self, initial_value: float = 0, axis: int = -1) -> torch.Tensor:
        """See :func:`k2.ragged.logsum`."""
        return _k2.logsum_per_sublist(self.ragged, initial_value, axis)

    def sum(self, initial_value: float = 0, axis: int = -1) -> torch.Tensor:
        """See :func:`k2.ragged.sum`."""
        return _k2.sum_per_sublist(self.ragged, initial_value, axis)

    def argmax(self, initial_value: float = 0, axis: int = -1) -> torch.Tensor:
        """See :func:`k2.ragged.argmax`."""
        return _k2.argmax_per_sublist(self.ragged, initial_value, axis)

    def sort(
        self, descending: bool = False, need_new2old_indexes: bool = False,
        axis: int = -1
    ) -> Optional[torch.Tensor]:
        """See :func:`k2.ragged.sort`."""
        return _k2.sort_sublists(
            self.ragged, descending, need_new2old_indexes, axis
        )

    def remove_axis(self, axis: int) -> "Ragged":
        """See :func:`k2.ragged.remove_axis`."""
        return Ragged(_k2.remove_axis(self.ragged, axis))

    def remove_values_eq(self, target: int) -> "Ragged":
        """See :func:`k2.ragged.remove_values_eq`."""
        return Ragged(_k2.remove_values_eq(self.ragged, target))

    def remove_values_leq(self, cutoff: int) -> "Ragged":
        """See :func:`k2.ragged.remove_values_leq`."""
        return Ragged(_k2.remove_values_leq(self.ragged, cutoff))
...
class RaggedFloat(Ragged):
    ...
    def __init__(
        self,
        ragged: Union[str, List[List[float]], _k2.RaggedFloat, _k2.RaggedShape],
...
class RaggedInt(Ragged):
    def __init__(
        self,
        ragged: Union[str, List[List[int]], _k2.RaggedInt, _k2.RaggedShape],
        values: Optional[torch.Tensor] = None,
    ):
