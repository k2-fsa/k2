...
def create_ragged_shape2(row_splits: Optional[torch.Tensor] = None,
                         ...
                         cached_tot_size: Optional[int] = -1) -> _k2.RaggedShape:
    '''
    ...
    Args:
      row_splits: Optionally, a torch.Tensor with dtype=torch.int32 and one axis
      row_ids: Optionally, a torch.Tensor with dtype=torch.int32 and one axis.
...
def compose_ragged_shapes(a: _k2.RaggedShape,
    ...
    Args:
       a: First shape to be composed
       b: First shape to be composed; b.dim0() must equal a.num_elements()
    Returns:
       an instance of _k2.RaggedShape with ans.num_axes() == a.num_axes() + b.num_axes() - 1.
