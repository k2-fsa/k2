...
class TestAddEpsilonSelfLoops(unittest.TestCase):
        ...
        for device in self.devices:
            for t in [0, 1]:
                ...
                if t:
                    ...
                    new_fsa = fsa.add_epsilon_self_loops()

                assert torch.all(
                    torch.eq(
                        new_fsa.arcs[:, :3],
                        torch.tensor([[0, 0, 0], [0, 1, 1], [0, 2, 1],
                                      [1, 1, 0], [1, 3, 2], [2, 2, 0],
                                      [2, 3, 3], [3, 3, 0], [3, 4, -1]],
                                     dtype=torch.int32,
                                     device=device)))
    ...
    def test_two_fsas(self):
        ...
        for device in self.devices:
            for t in [0, 1]:
                fsa1 = k2r.Fsa(s1).to(device)
                fsa2 = k2r.Fsa(s2).to(device)

                fsa1.requires_grad_(True)
                fsa2.requires_grad_(True)

                fsa_vec = k2r.Fsa.from_fsas([fsa1, fsa2])
                if t:
                    new_fsa_vec = k2.add_epsilon_self_loops(fsa_vec)
                else:
                    new_fsa_vec = fsa_vec.add_epsilon_self_loops()
                ...
                assert torch.allclose(
                    new_fsa_vec.scores,
                    torch.tensor([
                        0, 0.1, 0.2, 0, 0.3, 0, 0.4, 0, 0.5, 0, 0.1, 0.2, 0,
                        0.3, 0, 0.4
                    ]).to(device))
