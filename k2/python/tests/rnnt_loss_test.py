...
class TestRnntLoss(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.dtypes = [torch.float32, torch.float16]
        try:
            import torchaudio
        ...
        for device in self.devices:
            for dtype in self.dtypes:
                ...
                m = k2.mutual_information_recursion(
                    px=px.float(), py=py.float(), boundary=None
                )

                if device == torch.device("cpu") and dtype == torch.float32:
                    expected = -m
                assert torch.allclose(
                    -m, expected.to(device), atol=atol, rtol=rtol
                )
            ...
            for device in self.devices:
                for dtype in self.dtypes:
                    ...
                    if device.type == "cpu" and dtype == torch.float16:
                        continue
                    # lm: [B][S+1][C]
                    lm = lm_.to(dtype).to(device)
                    # am: [B][T][C]
                    am = am_.to(dtype).to(device)
        ...
        if self.has_torch_rnnt_loss:
            ...
            for dtype in self.dtypes:
                for device in self.devices:
                    ...
                    assert torch.allclose(
                        k2_grad.float(), torch_grad, atol=1e-2, rtol=1e-2
                    )
